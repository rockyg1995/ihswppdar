...
#define BAUDRATE 115200										// Baudrate in Bits per second (bps)
...
int main(void) {
	...
	while (1) {												// Infinite loop
		if (TCNT1 == OCR1A) {								// Display a String when Timer1 Matches OCR1A
			...
			USART_putstring("AT+CIPSTART=\"TCP\",\"api.thingspeak.com\",80");	// Connect API KEy
			USART_putstring("AT+CIPSEND=51");									// Send Serial Data
			USART_putstring("GET /update?key=YR8DHQMB2YJS3AAX&field1=outs\r\n");// Send Value
			USART_putstring("AT+CIPCLOSE");										// Close Data
		}
	}
	return 0;
}
